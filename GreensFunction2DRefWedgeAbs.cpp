...
//                 std::cout << "normal exit. n = " << n << " first term" << std::endl;
    ...
    const Real GreensFunction2DRefWedgeAbs::p_int_theta_second(const Real r,
                                                               const Real theta,
                                                               const Real t) const
    {
        const Real r_0(this->getr0());
        ...
        // 4 / (phi * a^2) * 
        // sum_n [
        //      phi / (n * pi) * sin(n pi theta / phi) * cos(n * pi / 2) * 
        //      sum_m[
        //           exp(-alpha_mn^2 * Dt) *
        //           4 * J_{n * pi / phi}(r0 * alpha_mn) * J_{n * pi / phi}(r * alpha_mn) / 
        //           (J_{n * pi / phi - 1}(a * alpha_mn) - J_{n * pi / phi + 1}(a * alpha_mn))^2 
        //           ]
        //       ]
        //
        Integer n(1);
        for(; n < num_out_term_use; ++n)
        {
            ...
            Real bessel_order(n * M_PI / this->phi);
            Integer m(1);
            ...
            for(; m < num_in_term_use; ++m)
            {
                ...
                Jnpp_r_alpha_mn     = gsl_sf_bessel_Jnu(bessel_order, r * alpha_mn);
                Jnpp_r0_alpha_mn    = gsl_sf_bessel_Jnu(bessel_order, r_0 * alpha_mn);
                Jnpp_d_1_a_alpha_mn = gsl_sf_bessel_Jnu(bessel_order - 1, a_alpha_mn);
                Jnpp_p_1_a_alpha_mn = gsl_sf_bessel_Jnu(bessel_order + 1, a_alpha_mn);
...
//                     std::cout << "normal exit. m = " << m << " second term" << std::endl;
            ...
            term = in_sum * sin(bessel_order * theta) * cos(n * M_PI / 2e0) / bessel_order;
            sum += term;

//             std::cout << "outer sum " << sum << ", term" << term << std::endl;

            if(fabs(in_sum / (bessel_order * sum)) < threshold)
            {
                /* if bessel_order * theta = product of natural number and pi, *
                 * term become zero and this code breaks too early. Therefore, *
                 * consider in_sum / bessel_order only here.                   */
