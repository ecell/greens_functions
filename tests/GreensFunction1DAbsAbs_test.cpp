...
//  std::cout << "DrawEventType gf.drawTime(0.5): " << t << std::endl;
  GreensFunction::EventKind eventType( gf.drawEventType(0.5, t) );

  BOOST_CHECK( ( eventType == GreensFunction::IV_ESCAPE || eventType == GreensFunction::IV_REACTION ) );
...
//when r0 = 1e-7, sigma = 2e-7, a = 4e-7, fluxratio > 1 and error detected.
  ...
  BOOST_CHECK_EQUAL(eventType, GreensFunction::IV_ESCAPE); //ESCAPE
  std::cout << "GreensFunction1DAbsAbs_test : test_DrawType(0.999999,t) = " << eventType << std::endl;

  eventType = gf.drawEventType(0.0, t);
  BOOST_CHECK_EQUAL(eventType, GreensFunction::IV_REACTION); //REACTION
